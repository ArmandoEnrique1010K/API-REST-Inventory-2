...
public interface MovementRepository extends JpaRepository<Movement, Long>, JpaSpecificationExecutor<Movement> {
    ...
    // Cantidad de movimientos realizados durante el dia de hoy
    @Query("""
            SELECT COUNT(m)
            FROM Movement m
            ...
            AND m.createdAt < :end
            """)
    long countByMovementsInDay(
            @Param("start") LocalDateTime start,
            @Param("end") LocalDateTime end);

    @Query("""
            SELECT m FROM Movement m JOIN FETCH m.user u JOIN FETCH m.model mo
            WHERE m.createdAt >= :start
            AND m.createdAt < :end
            """)
    List<Movement> findLastMovements(
            Pageable pageable,
            @Param("start") LocalDateTime start,
            @Param("end") LocalDateTime end);
}
