...
public interface DeliveryLineRepository
        ...
        // No quedan lineas activas pendientes
        @Query("""
                        SELECT CASE WHEN COUNT(dl) = 0 THEN true ELSE false END
                                                FROM DeliveryLine dl
                            WHERE dl.deliveryOrder.id = :deliveryOrderId
                              AND dl.lineStatus IN ('LINE_PENDING', 'LINE_EXCEEDED', 'LINE_READY')
                        """)
        boolean allLinesClosed(@Param("deliveryOrderId") Long deliveryOrderId);
        ...
        @Query("""
                        SELECT CASE
                            WHEN COUNT(dl) > 0 AND
                                 SUM(CASE WHEN dl.lineStatus <> 'LINE_CANCELED' THEN 1 ELSE 0 END) = 0
                            THEN true
                            ELSE false
                        END
                        FROM DeliveryLine dl
                        WHERE dl.deliveryOrder.id = :deliveryOrderId
                        """)
        boolean allLinesCanceled(Long deliveryOrderId);



        long countByDeliveryOrderId(Long id);

        boolean existsByDeliveryOrderIdAndLineStatusNot(Long id, LineStatus status);
        ...
        // FETCHRELATIONS EN EL SPECIFICATION (SOLAMENTE SI SE QUIERE ACCEDER AL SEGUNDO
        // NIVEL DE RELACION DE ENTIDAD COMO EL CASO DE "region")
